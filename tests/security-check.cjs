...
const required = [
  ...
  ['in-panel deletion confirmation', /id="dpe-confirm-overlay" role="dialog" aria-modal="true"/],
  ['confirmation button fails closed', /id="dpe-confirm-start" class="danger" type="button" disabled/],
  ['exact in-panel phrase gate', /entered === pending\.phrase/],
  ['deletion request diagnostics', /debugLog\('delete-request'/],
  ['deletion response diagnostics', /debugLog\('delete-response'/],
  ['visible deletion progress', /Deleted \$\{runState\.deleted\.toLocaleString\(\)\} total/],
  ...
  ['one-click scan/delete loop', /function startCleanupLoop/],
  ['one-click control', /id="dpe-cleanup-loop"/],
  ['newest-first fresh default', /deleteOrder: 'newest'/],
